...
pts = np.array([[1211, 265], [1797, 349],
                [1660, 1076], [733, 1069],[57, 691]],
               np.int32)
...
while cv2.waitKey(1) < 1:
    ...
    #Copia del frame sobre el cual se dibuja
    aux = np.copy(frame)
    #Dibujo del ROI
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        res = tex.readtext(gran, allowlist = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', rotation_info=[0, 2, 4, 5, 7, 10])
        #print('res: ', res)
        if len(res) >= 1 and res[-1][2] > 0.4:
            ...
            if  len(text) >= 6:
                ...
                if (temp != valor_f) & (temp != '') & (count2 >= 6):
                    cv2.imwrite(r"C:\Users\Usuario\Documents\heatmap_application\grafana\images\placas\img" + str(count) + ".jpg", gran)
                    count += 1
                temp = valor_f
                count2 += 1
    end = datetime.utcnow()
